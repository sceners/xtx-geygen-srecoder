...
push_noSDR MACRO adresse
  mov eax, adresse - 1
  inc eax
  push eax
ENDM

mov_noSDR MACRO adresse1, adresse2
  mov eax, adresse2 - 1
  inc eax
  mov adresse1, eax
ENDM 
...
Table_Rand			db "9876543210",0
Table_Rand2			db "9876543210",0
Table_Rand3			db "5678901234",0
Table_Rand4			db "12345678",0
Table_Rand5			db "5678901234",0
Table_Rand6			db "456789",0
Table_RandB			db "123456789",0
Table_RandB2			db "23456789",0
Table_RandB3			db "543987621",0


Court		db	"5 chars minimum ! ",0
...
DoKey	proc	hWnd:DWORD, uMsg:DWORD, wParam:DWORD, lParam:DWORD
		...
		;invoke SetDlgItemText,hWnd,EditSerial,chr$("Sinon le calcul sur  votre Nom est éroné ....")
		call InitCheck
		xor eax,eax
		invoke GetTickCount
		mov Tick,eax
	...
	jmp @N1
	@@:
		push 0Ah   ; Pousse sur la pile le nombre de carac de la Table en Hexa
		call Randomize
		...
		inc ecx
	@N1:
		cmp ecx,5           ;  verifie qu'il y a bien 6 carac en ecx 
	jnz @b
		mov SerialRND1,edi
		
		
		xor eax,eax                          ;======\
		xor ecx,ecx                          ;                \====== Restauration de la pile
		xor edx,edx                          ;               /
		xor ebx,ebx                          ;======/
...
call InitCheck
	...
	@@:
		push 0Ah   ; Pousse sur la pile le nombre de carac de la Table en Hexa
		call Randomize
		movsx eax,byte ptr ds:[eax+Table_Rand5]	;appel le random sur des carac de la Table
		mov byte ptr ds:[ecx+edi],al		;met les carac dans un buffer ecx
		inc ecx
	@N2:
		cmp ecx,5           ;  verifie qu'il y a bien 6 carac en ecx 
...
;		
		...
		XOR EBX,EBX
		invoke GetDlgItemText,hWnd,IDC_NAME,addr NameBuffer,80
...
;		INC EDI
;		LOOPD @b
;====================================== boucle1 xor sur chaque carac du nom fin ===================================
...
;		mov dword ptr ds:[buff1],eax
		...
		CDQ
		IDIV  ECX
		call convert_Hex2Ascii 
		mov byte ptr ds:[carac1],dl
		
		

		movsx eax,byte ptr ds :[edi+1]
		OR eax,45h   
		MOV     ECX,0Ah
		CDQ
		IDIV  ECX
		call convert_Hex2Ascii 
		mov byte ptr ds:[carac2],dl
		...
		IDIV  ECX
		call convert_Hex2Ascii 
		mov byte ptr ds:[carac4],dl

		MOV ECX,DWORD PTR DS:[buffcont]
;	XOR EBX,EBX
	pop ecx
		mov esi,edi
@progress_0040113B:   ;******************* ADDITIONNE LES CARAC HEXA DU  NOM 

        XOR EDX,EDX
        MOV DL,BYTE PTR DS:[ESI]
        ADD EBX,EDX
        INC ESI
LOOPD @progress_0040113B ;*********************************************************
        mov eax,ebx
        MOV     ECX,0Ah
       CDQ
      IDIV  ECX
      call convert_Hex2Ascii 
      mov byte ptr ds:[carac5],dl
...
;		jnz @Loop;  boucle jusqu a ce que ces 2 valeurs  soient egales
		...
		invoke wsprintf,addr szSerial,SADD("%d%d%d%d%d%s%s"),[carac1], [carac2],[carac3],[carac4], [carac5],[SerialRND1],[SerialRND5];   *****  58556
	...
	@Maj:
		...
		invoke lstrcpy,addr szAboutMsg,addr szPatchSuccess
		invoke GetWindowRect,hWnd,addr PosRect
		mov AboutFlag,0
		invoke DialogBoxParam,hInstance,AboutDlg,hWnd,addr AboutProc,NULL
;		invoke SetDlgItemText, hWND, IDC_NAME , addr Maj 
		ret
	@Court:
		invoke SetDlgItemText, hWND, IDC_NAME , addr Court
		ret
	@Vide:
		invoke SetDlgItemText, hWND, IDC_NAME , addr Vide	
		ret
...
DestroyKey	proc
		...
		ret
DestroyKey EndP
convert_Hex2Ascii proc
;***********************************************************************	CONVERTION EXEMPLE 33(hexa)=3(Ascii)***********************	
			add     al,40h
			cbw
			and     ah,09h
			add     al,ah
